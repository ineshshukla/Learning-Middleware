...
export default function ModuleViewerPage() {
  ...
  const [allModules, setAllModules] = useState<Module[]>([]); // All course modules for sidebar
  const [moduleContent, setModuleContent] = useState<string>("");
  const [currentPage, setCurrentPage] = useState(0); // Pagination for content
  const [contentPages, setContentPages] = useState<string[]>([]); // Split content into pages
  ...
  // Split content into pages based on ## headers or paragraphs
  const splitContentIntoPages = (content: string): string[] => {
    ...
    if (sections.length > 1) {
      return sections; // If we have sections, use them as pages
    }
    
    // Otherwise, split by paragraphs (every ~500 words)
  ...
  // Update content pages whenever moduleContent changes
  useEffect(() => {
    if (moduleContent) {
  ...
  const initializeModule = async () => {
    try {
      ...
      setAllModules(modules); // Store all modules for sidebar
